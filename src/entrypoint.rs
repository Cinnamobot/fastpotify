...
pub(crate) fn run() -> eframe::Result<()> {
    ...
    let default_filter = if cli.verbose {
        // `--verbose` carries the engine's own decisions, not only this
        // crate's. The crossfade and preload handshake is the one part of
        // playback a host cannot see from its side: whether the player
        // decided to preload, and whether it had something to mix in, are
        // both decisions taken inside `librespot-playback`, and a transition
        // that never fires is otherwise indistinguishable from one that was
        // never planned.
        "info,librespot=info,librespot_playback=debug,librespot_connect=debug,fastpotify=debug,spotifast=debug"
    } else {
